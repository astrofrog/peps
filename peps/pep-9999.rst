...
With current packaging standards, maintainers have the choice between either
making one of the backends or frontends be always required, or requiring users
to always specify extras, e.g. ``package[backend]`` and therefore risking having
users have an unusable installation if they simply install ``package``. Having a
...
and this PEP does not change this.
...
``default-optional-depencencies`` with the following description:

* `TOML <https://toml.io/>`_ type: Array of strings
* Corresponding core metadata field: ``Default-Extra``

Each string in ``default-optional-depencencies`` should be the name of an extra
